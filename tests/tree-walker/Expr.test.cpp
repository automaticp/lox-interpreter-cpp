...
TEST_CASE("unary") {
    ...
    UnaryExpr u_not_true{
        Token{ bang, "!", 0 },
        IExpr::make_unique<LiteralExpr>(Token{ kw_true, "true", 0, true})
    };

    UnaryExpr u_minus_num{
        Token{ minus, "-", 0 },
        IExpr::make_unique<LiteralExpr>(Token{ number, "5.6", 0, 5.6 })
    };
...
TEST_CASE("binary") {
    ...
    BinaryExpr b_num_times_num{
        Token{ star, "*", 0 },
        IExpr::make_unique<LiteralExpr>(Token{ number, "5.6", 0,5.6 }),
        IExpr::make_unique<LiteralExpr>(Token{ number, "0.1", 0, 0.1 })
    };

    BinaryExpr b_false_and_true{
        Token{ kw_and, "and", 0 },
        IExpr::make_unique<LiteralExpr>(Token{ kw_false, "false", 0, false }),
        IExpr::make_unique<LiteralExpr>(Token{ kw_true, "true", 0, true})
    };

    BinaryExpr b_hello_plus_world{
        Token{ plus, "+", 0 },
        IExpr::make_unique<LiteralExpr>(Token{ string, "\"hello\"", 0, "hello" }),
        IExpr::make_unique<LiteralExpr>(Token{ string, "\"world\"", 0, "world" })
    };
...
TEST_CASE("group") {
    ...
    GroupedExpr g_nil{
        IExpr::make_unique<LiteralExpr>(Token{ TokenType::kw_nil, "nil", 0, nullptr })
    };
...
TEST_CASE("compound") {
    ...
    BinaryExpr c{
        Token{ star, "*", 0},
        IExpr::make_unique<UnaryExpr>(
            Token{ minus, "-", 0 },
            IExpr::make_unique<LiteralExpr>(Token{ number, "123", 0, 123.0 } )
        ),
        IExpr::make_unique<GroupedExpr>(
            IExpr::make_unique<LiteralExpr>(Token{ number, "45.67", 0, 45.67 })
        )
    };
